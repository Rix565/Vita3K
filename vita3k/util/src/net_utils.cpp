...
bool download_file(std::string url, const std::string &output_file_path, ProgressCallback progress_callback) {
    ...
    CurlDownloadCallback curl_callback = +[](void *c, long t, long d) {
        const auto data = (CallbackData *)c;
        ...
        if (!data->second)
            return 0;
        const auto progress_percent = static_cast<float>(d) / static_cast<float>(t) * 100.0f;

        const auto elapsed_time_ms = std::difftime(get_current_time_ms(), data->first);

        // Calculate remaining time in seconds
        const auto remaining_bytes = static_cast<double>(t - d);
        const auto remaining_time = static_cast<uint64_t>((remaining_bytes / d) * elapsed_time_ms) / 1000;

        ProgressState *callback_result = data->second(progress_percent, remaining_time);

        while (callback_result->pause) {
        ...
        // Also check here just in case pause was set to false first
        if (!callback_result->download) {
            return 1; // Returning anything thats not 0 aborts the request
        }
        return 0;
